...
int main(int argc, char **argv)
{
  ...
  int num_transfers;
  const char *url_base = "https://cdn-3-dx.cdnpan.com/hls/2018/10/26/1ufRuyTk";
  const char *file_base = "out%d.ts";
  ...
  for(i = 0; i < num_transfers; i++) {
    char str[100];
    char filename[100];
    snprintf(str, 100, "%s/out%03d.ts", url_base, i);
    snprintf(filename, 100, "out%03d.ts", i);
    setup(&trans[i], i, str, filename);
